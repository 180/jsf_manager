...
public class PersonFormBean implements Serializable {
		...
		for (Person person : pm.getAllPersons()) {
			if (person.getPesel().equalsIgnoreCase(pesel)) {
				FacesMessage message = new FacesMessage(
						"Osoba o takim peselu juz istnieje");
	...
	public void validateName(FacesContext context, UIComponent component, Object value){
		...
		if (nameExists) {
			FacesMessage message = new FacesMessage();
			message.setDetail("Istnieje juz osoba o takim imieniu");
			message.setSummary("Istnieje juz osoba o takim imieniu");
			message.setSeverity(FacesMessage.SEVERITY_ERROR);
			throw new ValidatorException(message);
		}
	}
